...
names.id = "names";
names.innerText =
  "a project by Lennart Dolderer, Julian Hoch, Elias Röhner, Timo Sawitzky";
...
uni.id = "uni";
uni.innerText =
  "Supervision: Tenure-Track Prof. Dr. Thomas Wortmann  |  Tutors: Zuardin Akbar, Gili Ron  |  WS23/24  |  B.Sc. Architecture and Urban Planning  |  5th semester";
...
logoManual.style.position = "absolute"; // or 'absolute', depending on your layout needs
logoManual.style.top = "47%";
logoManual.style.left = "50%";
logoManual.style.transform = "translate(-50%, -50%)";
...
function main() {
  ...
  testInputFolder.add(parameters, "elevation", 1, 100, 1);
...
function generatorStarting() {
  ...
  partList = barLengths; //[5.5,8,8,8,8,8,8,8,8,3,3,3,3,3,3];///für Elias hier array barLengths = partlist einfügen
...
function generateBranchConnectionAlongLineAB(
  usedParts,
  diagonalStartingPoint,
  endVector,
  distanceEndtoEnd
) {
  ...
  if (collisionCheck) {
    ...
    for (let i = 0; i < usedParts.length; i++) {
      ...
      let newVector = directionalVector
        .clone()
        .normalize()
...
function testForPosibility(testInput) {
  let totalLengthCalculated = testInput.reduce(
  ...
  filterInputHighFlat.push(
    testInput.filter((testInput) => testInput >= tableHeightNr)
  );
  ...
  let filterInputLow = testInput.filter(
    (testInput) => testInput < tableHeightNr
  );
  let shortRemainList = filterInputLow;

  let checkRemain = legCountNr - filterInputHigh.length;
  let longRemainList = longSortedInput.splice(
    legCountNr,
    Math.abs(checkRemain)
  );
  //longSortedInput behält alle Linien welche für die beine genutzt werden können

  if (checkRemain > 0) {
    let sumFilterInputLow = filterInputLow.reduce(
      (accumulator, currentValue) => accumulator + currentValue,
      0
    );
  ...
  remainingParts = checkRemainingParts(
    shortRemainList,
    shortUsedList,
    longRemainList
  );
...
function createSurface(divisions) {
  ...
  const upperSurfaceMaterial = new THREE.MeshBasicMaterial({
    color: 0xff0000,
    side: THREE.DoubleSide,
    wireframe: false,
  });
  const upperSurface = new THREE.Mesh(
    upperSurfaceGeometry,
...
function surfaceDevisionThree(count, surface) {
  ...
  const upperSurface1 = new THREE.Mesh(
    upperSurfaceGeometry1,
    upperSurfaceMaterial
  );
  ...
  const upperSurface2 = new THREE.Mesh(
    upperSurfaceGeometry2,
    upperSurfaceMaterial
  );
  ...
  const upperSurface3 = new THREE.Mesh(
    upperSurfaceGeometry2,
    upperSurfaceMaterial
  );
  upperSurface3.rotation.x = -Math.PI / 2;
  upperSurface3.position.set(
    -(halfHeight / 2),
    tableHeightNr,
    -(halfWidth / 2)
  );
...
function surfaceDevisionFour(count, surface) {
  ...
  const upperSurface4 = new THREE.Mesh(
    upperSurfaceGeometry1,
    upperSurfaceMaterial
  );
...
function placePointsBottom(startingPoints) {
  let testlength = partList.reduce(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );
  ...
  for (let i = 0; i < legCountNr; i++) {
    ...
    if (Array.isArray(sortedInput[i])) {
      ...
      let randomPointInbetween = generateRandomBasePoints(
        center,
        b,
        tableHeightNr - c
      );
      ...
      let arrayInternBotoom = arrayIntern.filter(
        (arrayIntern) => arrayIntern > decissionHeight
      );
      let arrayInternTop = arrayIntern.filter(
        (arrayIntern) => arrayIntern < decissionHeight
      );
      ...
      for (let j = 0; j < arrayInternBotoom.length; j++) {
        ...
        let randomPointBottomBranch = generateRandomBasePoints(
          randomPointInbetween,
          bBottom,
          0
        );
        const sphereGeometry = new THREE.SphereGeometry(sphereDiameter);
      ...
      for (let k = 0; k < arrayInternTop.length; k++) {
        ...
        let randomPointTopBranch = generateRandomBasePoints(
          randomPointInbetween,
          bTop,
          tableHeightNr
        );
...
function generateBranchConnectionAB(startingPoints, bottomPoints) {
  const cylinderY = createCylinderBetweenPoints(
    startingPoints,
    bottomPoints,
    0.1,
    8
...
function createCylinderBetweenPoints(
  ...
  const height = direction.length() - 1;
  // console.log(height);
  ...
  const cylinderMaterialShort = new THREE.MeshPhysicalMaterial({
    color: 0xf90000,
    wireframe: true,
  });

  let cylinderShortA = new THREE.Mesh(
    cylinderGeometryShort,
    cylinderMaterialShort
  );
  cylinderShortA.position.copy(startPoint);
  cylinderShortA.position.addScaledVector(
    newVectorShortA.normalize(),
    anschluss / 2
  );
  cylinderShortA.quaternion.setFromUnitVectors(
    new THREE.Vector3(0, 1, 0),
    direction.clone().normalize()
  );
  ...
  let cylinderShortB = new THREE.Mesh(
    cylinderGeometryShort,
    cylinderMaterialShort
  );
  cylinderShortB.position.copy(endPoint);
  cylinderShortB.position.addScaledVector(
    newVectorShortB.normalize(),
    anschluss / 2
  );
  cylinderShortB.quaternion.setFromUnitVectors(
    new THREE.Vector3(0, 1, 0),
    directionCounter.clone().normalize()
  );
  ...
  let finalTubexmeshA = CSG.toMesh(
    bspResultA,
    cylinderShortA.matrix,
    cylinderShortA.material
  );
  ...
  let finalTubexmeshB = CSG.toMesh(
    bspResultB,
    cylinderShortB.matrix,
    cylinderShortB.material
  );
  ...
  fontPromise.then((font) => {
    ...
    createTextGeometry(
      "stick_" +
        diagonaleCylinder.indexOf(cylinder) +
        " length: " +
        cylinder.geometry.parameters.height.toFixed(2) +
        " cm",
      font,
      textParameters
    ).then((textGeometry) => {
  ...
  for (let i = 0; i < allsphere.length; i++) {
    for (let j = 0; j < allcylindermitLoch.length; j++) {
      ...
      if (collision) {
        ...
        var joint = CSG.toMesh(
          sphereAndShortCylinder,
          allsphere[i].matrix,
          material
        );
    ...
    fontPromise.then((font) => {
      // Create text 1
      createTextGeometry(
        "Joint_" + allsphere.indexOf(allsphere[i]),
        font,
        textParameters
      ).then((textGeometry) => {
...
function createDiagonalConnections(remainingParts, allVectorsArray) {
  ...
  let potentialEndPoints = filterVectorsByY(
    diagonalStartingPoint,
    vectorsForDiagonal
  );
  ...
  if (remainingLength > distance) {
    const { resultArray, remainingValues } = optimalLength(
      remainingPartsLokal,
      distance
    );
